...
void loop() {
  // const char text[] = "Hello, NRF24L01P";
  // radio.write(&text, sizeof(text)); //해당 메시지를 수신자에게 보냄
  int xValue, yValue;
  uint8_t data[4];

  // X, Y 값을 uint8_t(2바이트씩)로 변환하여 저장
  data[0] = xValue & 0xFF;  // X축 하위 바이트
  data[1] = (xValue >> 8) & 0xFF;  // X축 상위 바이트
  data[2] = yValue & 0xFF;  // Y축 하위 바이트
  data[3] = (yValue >> 8) & 0xFF;  // Y축 상위 바이트
  ...
  //Serial.println("Message Sent Succesuful!");
